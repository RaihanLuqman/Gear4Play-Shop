<?php
// Memulai sesi untuk menangani cart_id
session_start();
...
$id_product = isset($_GET['id']) ? intval($_GET['id']) : 0; // Memastikan id produk valid
$product_query = "SELECT * FROM product WHERE id_product = $id_product";
$product_result = $conn->query($product_query);

// Mengalihkan jika produk tidak ditemukan
...
// Memastikan cart_id ada dalam sesi
if (!isset($_SESSION['cart_id'])) {
    $_SESSION['cart_id'] = uniqid(); // Membuat cart_id unik jika belum ada
}
?>
            ...
            <section class="detail_products">
                ...
                <div class="cart-section">
                    ...
                    <form action="checkout.php" method="GET">
                        <input type="hidden" name="id_product" value="<?php echo htmlspecialchars($product['id_product']); ?>">
                        <input type="hidden" name="quantity" id="qty-checkout" value="1">
                        <button type="submit">Beli</button>
                    </form>
                    <form action="add_to_wishlist.php" method="POST">
                        <input type="hidden" name="id_product" value="<?= $product['id_product']; ?>">
                        <input type="hidden" name="active" value="1"> <!-- 1 untuk menambahkan, 0 untuk menghapus -->
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        function updateSubtotal() {
            ...
            // Update semua input hidden untuk quantity
            document.getElementById("qty-cart").value = qty;
            document.getElementById("qty-checkout").value = qty;
        }
